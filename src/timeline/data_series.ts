...
/**
 * no need use , will use startTime to filter the cache
 */
export const MAX_STATS_DATA_POINT_BUFFER_SIZE = 1000 * 3;
...
export class TimelineDataSeries {
  constructor(statsType) {
    ...
    this.boundariesMap = new Map<string, any>();
  }
   ...
   */
  addPoint(value, timeTicks = new Date().getTime()) {
  ...
  getValuesInternal_(startTime, stepSize, count) {
    ...
    for (let i = 0; i < count; ++i) {
      while (
        ...
        (this.dataPoints_[nextPoint]?.time || 0) < time
      ) {
